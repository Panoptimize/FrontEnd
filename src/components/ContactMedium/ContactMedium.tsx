...
const ContactMedium: React.FC<IContactMedium> = ({ data: customData }) => {
      ...
      datasets: [
        {
          ...
          backgroundColor: [    // Add more colors if more labels are added
            '#008F89',
            '#BBDFE4',
            '#8FA8A7',
          ],
    ...
    const totalSum = data.datasets[0].data.reduce((acc, current) => acc + current, 0);
    ...
    return(
      <div className="bg-white shadow-md rounded-3xl p-1 flex flex-col items-center justify-center w-full max-w-md" style={{ minWidth: '250px', width: '250px', height: '230px' }}>
        <div className="text-md text-left pt-2 px-3 pb-2 w-full">Contact Medium</div>
            <div className="text-3xl text-left font-semibold px-5 pb-0 w-full">
                {totalSum}
            </div>
        <div className="flex-grow w-full pb-6 px-2" style={{ minHeight: '140px', maxHeight: '180px' }}>
